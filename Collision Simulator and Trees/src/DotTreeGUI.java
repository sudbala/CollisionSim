...
public class DotTreeGUI extends DrawingGUI {
	...
	private int testFind(int x, int y, int r, int expectedCircleRectangle, int expectedInCircle, int expectedHits) {
		...
		//System.out.println("The number of hits achieved is " + num + " should be " + expectedHits);
		if (num != expectedHits) {
			errs++;
	...
	private void test0() {
		...
		int bad = 0;
		bad += testFind(0,0,900,3,3,3);
		//System.out.println(tree.allPoints()); // rect for all; circle for all; find all
	...
	private void test1() {
		...
		tree.insert(new Dot(150,450)); // B
		//System.out.println("allPoints list1 is" + tree.allPoints());
		//System.out.println("inserted a dot");
		tree.insert(new Dot(250,550)); // C
		//System.out.println("allPoints list1 is" + tree.allPoints());
		//System.out.println("size should be 2 and it's" + tree.size());
		tree.insert(new Dot(450,200)); // D
		//System.out.println("size should be 3 and it's" + tree.size());
		tree.insert(new Dot(200,250)); // E
		//System.out.println("size should be 4 and it's" + tree.size());
		tree.insert(new Dot(350,175)); // F
		//System.out.println("size should be 5 and it's" + tree.size());
		tree.insert(new Dot(500,125)); // G
		//System.out.println("allPoints list2 is" + tree.allPoints());
	...
	private void test2() {
		...
		tree = new PointQuadtree<Dot>(new Dot(300,400), 0,0,800,600);
		tree.insert(new Dot(50,50)); 
		tree.insert(new Dot(45,45)); 
		tree.insert(new Dot(100,100)); 
		tree.insert(new Dot(150,150)); 
		tree.insert(new Dot(165,165));
		
		tree.insert(new Dot(90,90)); 
		tree.insert(new Dot(95,95)); 
		tree.insert(new Dot(400,600)); 
		int bad = 0;
		bad += testFind(150,450,10,3,1,0);
		if (bad==0) System.out.println("test 1 passed!");
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		//g.drawOval((int)tree.getPoint().getX()-dotRadius/2, (int)tree.getPoint().getY()-dotRadius/2, dotRadius, dotRadius);
		g.fillOval((int)tree.getPoint().getX()-dotRadius/2, (int)tree.getPoint().getY()-dotRadius/2, dotRadius, dotRadius);
		
		g.drawLine((int)tree.getX1(), (int)tree.getPoint().getY(),(int) tree.getX2(),(int) tree.getPoint().getY());
		g.drawLine((int)tree.getPoint().getX(), (int)tree.getY1(), (int)tree.getPoint().getX(),(int) tree.getY2());
		//System.out.println("x1 is " +tree.getX1()+" y1 is " +tree.getY1()+" x2 is " +tree.getX2()+" Y2 is " +tree.getY2());
		//g.drawRect((int)tree.getX1(), (int)tree.getPoint().getY(),(int) tree.getX2(),(int) tree.getPoint().getY());
		
//		g.drawLine((int) tree.getPoint().getX(),(int) tree.getPoint().getY(), tree.getX2(), (int) tree.getPoint().getY());
//		g.drawLine((int) tree.getPoint().getX(),(int) tree.getPoint().getY(), (int)tree.getPoint().getX(), (int) tree.getY1());
//		g.drawLine((int) tree.getPoint().getX(),(int) tree.getPoint().getY(), tree.getX1(), (int) tree.getPoint().getY());
//		g.drawLine((int) tree.getPoint().getX(),(int) tree.getPoint().getY(), (int)tree.getPoint().getX(), (int) tree.getY2());
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				new DotTreeGUI();
				//new DotTreeGUI().test2();
				//new DotTreeGUI().test0();
				//new DotTreeGUI().test0();
